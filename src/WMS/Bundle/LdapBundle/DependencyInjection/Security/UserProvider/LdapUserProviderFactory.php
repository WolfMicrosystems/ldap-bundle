<?php
namespace WMS\Bundle\LdapBundle\DependencyInjection\Security\UserProvider;
...
use WMS\Library\Ldap\Enum as LdapEnum;
...
class LdapUserProviderFactory implements UserProviderFactoryInterface
{
    public function create(ContainerBuilder $container, $id, $config)
    {
        ...
        $definition->replaceArgument(4, $config['refresh_credentials']['every_x_requests']);
        $definition->replaceArgument(5, $config['refresh_credentials']['every_x_seconds']);
    ...
    public function addConfiguration(NodeDefinition $builder)
    {
        $builder
            ...
            ->children()
                ...
                ->end()
                ->scalarNode('class')
                    ->defaultValue('WMS\Bundle\LdapBundle\Security\Ldap\User\LdapUser')
                    ->cannotBeEmpty()
                    ->validate()
                        ->ifTrue(function ($v) { return !is_subclass_of($v, 'WMS\Bundle\LdapBundle\Security\Ldap\User\LdapUserInterface'); })
                        ->thenInvalid('"%s" does not implement WMS\Bundle\LdapBundle\Security\Ldap\User\LdapUserInterface')
                    ->end()
                ->end()
                ->enumNode('username_form')
                    ...
                    ->cannotBeEmpty()
                ->end()
                ->arrayNode('refresh_credentials')
                    ->children()
                        ->integerNode('every_x_requests')->min(0)->defaultValue(1)->treatNullLike(0)->end()
                        ->integerNode('every_x_seconds')->min(0)->defaultNull()->treatNullLike(0)->end()
                    ->end()
                ->end()
                ->arrayNode('groups')
                    ->addDefaultsIfNotSet()
